...
document.addEventListener('DOMContentLoaded', function() {
    // Prices combination from PHP
    var prices = <?php echo json_encode($prices); ?>;

    // Update the price based on selected size and color
    function updateCartPrice(index) {
        var priceInput = document.getElementById('item-price-' + index);
        var color = priceInput.getAttribute('data-color');
        var size = priceInput.getAttribute('data-size');
        var quantity = document.getElementById('quantity-input-' + index).value;

        if (prices[color] && prices[size]) {
            var price = prices[color][size];
            var displayPrice = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(price);
            var totalPrice = price * quantity;
            var displayTotalPrice = new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(totalPrice);
            
            document.querySelector('.item-price[data-index="' + index + '"]').value = price;
            document.querySelector('.display-price').textContent = displayPrice;
            document.querySelector('.total-price').textContent = displayTotalPrice;
        }
    }

    // Attach event listeners to the color and size dropdowns in the cart
    var colorSelect = document.getElementById('color');
    var sizeSelect = document.getElementById('size');

    if (colorSelect && sizeSelect) {
        colorSelect.addEventListener('change', function() {
            var index = colorSelect.getAttribute('data-index');
            updateCartPrice(index);
        });

        sizeSelect.addEventListener('change', function() {
            var index = sizeSelect.getAttribute('data-index');
            updateCartPrice(index);
        });
    }
});
